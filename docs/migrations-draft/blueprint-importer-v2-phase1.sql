-- =====================================================================
-- Blueprint Importer v2 — Phase 1 migration DRAFT (NOT YET APPLIED)
-- =====================================================================
-- Intended path on apply: supabase/migrations/20260604043421_2f153e21-f518-452d-bfab-e429bc0d7e47.sql
--
-- This file is a reviewable draft. Lovable's platform blocks direct writes
-- into supabase/migrations/, so the SQL is parked here until explicitly
-- approved. When approved, hand this body to the supabase migration tool
-- verbatim to create the timestamped migration file and apply it.
--
-- Phase 1 scope: schema only. No runtime endpoint, worker, or estimate
-- behavior is wired in this phase. Tables here back future
-- detection / acceptance / draft generation work.
--
-- Phase 0 docs that define the rules these tables enforce:
...
CREATE TABLE IF NOT EXISTS public.blueprint_import_sessions (
  ...
  CONSTRAINT blueprint_import_sessions_status_chk CHECK (
    status IN (
      'draft','parsed','trades_detected','user_review_required',
...
CREATE POLICY "blueprint_import_sessions tenant read"
  ON public.blueprint_import_sessions FOR SELECT TO authenticated
  USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "blueprint_import_sessions tenant insert"
  ON public.blueprint_import_sessions FOR INSERT TO authenticated
  WITH CHECK (tenant_id = public.get_user_tenant_id());
CREATE POLICY "blueprint_import_sessions tenant update"
  ON public.blueprint_import_sessions FOR UPDATE TO authenticated
  USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "blueprint_import_sessions tenant delete"
  ON public.blueprint_import_sessions FOR DELETE TO authenticated
  USING (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_source_documents (
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_src_doc_type_chk CHECK (
    document_type IN (
      'roof_report','wall_report','blueprint_set','spec_book','addendum','unknown'
    )
  ),
  CONSTRAINT bp_src_provider_chk CHECK (
    provider IN (
      'roofr','eagleview','internal_geometry','user_uploaded_blueprint','unknown'
    )
  ),
  CONSTRAINT bp_src_extraction_status_chk CHECK (
    extraction_status IN ('pending','in_progress','succeeded','failed','skipped')
  )
);
...
CREATE POLICY "blueprint_source_documents tenant all"
  ON public.blueprint_source_documents FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_detected_trades (
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_det_support_chk CHECK (
    support_status IN ('mvp_supported','measurement_object_only','future_supported','unsupported')
  ),
  CONSTRAINT bp_det_confidence_chk CHECK (confidence >= 0 AND confidence <= 1),
  CONSTRAINT bp_det_status_chk CHECK (
    status IN ('detected','dismissed','superseded','promoted')
  )
);
...
CREATE POLICY "blueprint_detected_trades tenant all"
  ON public.blueprint_detected_trades FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_accepted_trades (
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_acc_status_chk CHECK (
    status IN ('accepted','rejected','superseded')
  ),
  CONSTRAINT bp_acc_review_state_chk CHECK (
    review_state IN ('pending_review','blocked','cleared','manual_only')
  ),
  -- Phase 0 rules (defense in depth):
  --   * windows_doors is measurement_object_only — never an accepted top-level MVP trade
  --   * future-only trades may only enter as manual_only
...
CREATE POLICY "blueprint_accepted_trades tenant all"
  ON public.blueprint_accepted_trades FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_plan_paths (
  ...
  confidence            NUMERIC(4,3) NOT NULL DEFAULT 0,
  created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_pp_path_type_chk CHECK (
    path_type IN ('report_page','blueprint_sheet','spec_section','user_entry','derived')
  ),
...
CREATE POLICY "blueprint_plan_paths tenant all"
  ON public.blueprint_plan_paths FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE POLICY "blueprint_measurement_objects tenant all"
  ON public.blueprint_measurement_objects FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_review_flags (
  ...
  CONSTRAINT bp_rf_entity_type_chk CHECK (
    related_entity_type IN (
      'import_session','source_document','detected_trade','accepted_trade',
      'measurement_object','template_binding','material_draft_line','labor_draft_line','plan_path'
    )
  )
);
...
CREATE POLICY "blueprint_review_flags tenant all"
  ON public.blueprint_review_flags FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_template_bindings (
  ...
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_tb_binding_status_chk CHECK (
    binding_status IN ('pending','ready','blocked','rejected','superseded')
  )
);
...
CREATE POLICY "blueprint_template_bindings tenant all"
  ON public.blueprint_template_bindings FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_material_draft_lines (
  ...
  status                      TEXT NOT NULL DEFAULT 'draft',
  created_at                  TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_mdl_catalog_status_chk CHECK (
    catalog_resolution_status IN ('unresolved','matched','ambiguous','missing','manual_override')
  ),
  CONSTRAINT bp_mdl_status_chk CHECK (
    status IN ('draft','ready','blocked','rejected','superseded')
  )
);
...
CREATE POLICY "blueprint_material_draft_lines tenant all"
  ON public.blueprint_material_draft_lines FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
...
CREATE TABLE IF NOT EXISTS public.blueprint_labor_draft_lines (
  ...
  status                  TEXT NOT NULL DEFAULT 'draft',
  created_at              TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT bp_ldl_status_chk CHECK (
    status IN ('draft','ready','blocked','rejected','superseded')
  )
);
...
CREATE POLICY "blueprint_labor_draft_lines tenant all"
  ON public.blueprint_labor_draft_lines FOR ALL TO authenticated
  USING (tenant_id = public.get_user_tenant_id())
  WITH CHECK (tenant_id = public.get_user_tenant_id());
