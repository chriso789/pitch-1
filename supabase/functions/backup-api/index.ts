// backup-api — routed Edge Function.
...
const app = createRouter("backup-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "backup-api", ok: true }));

// Apply auth + tenant guard to everything else unless explicitly public.
app.use("/*", requireAuth);
app.use("/*", requireTenant);

app.post("/snapshot", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.post("/restore", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
