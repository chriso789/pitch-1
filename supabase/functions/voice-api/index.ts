// voice-api — routed Edge Function.
...
const app = createRouter("voice-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "voice-api", ok: true }));

// Apply auth + tenant guard to everything else unless explicitly public.
app.use("/*", requireAuth);
app.use("/*", requireTenant);

app.post("/dial", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.post("/transcribe", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
