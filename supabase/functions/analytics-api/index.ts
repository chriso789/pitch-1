// analytics-api — routed Edge Function.
...
const app = createRouter("analytics-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "analytics-api", ok: true }));

// Apply auth + tenant guard to everything else unless explicitly public.
app.use("/*", requireAuth);
app.use("/*", requireTenant);

app.post("/event", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.post("/dashboard", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
