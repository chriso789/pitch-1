...
export const handle = async (req) => {
  ...
  try {
    ...
    // Supabase secrets. NEVER returns or logs the password value.
    if (action === "sandbox_test_login_status") {
      ...
      return json({
        ...
        username: username ?? null,            // safe to display
    ...
    // ---------------- sync_accounts ----------------
    // Post-OAuth hydration: pull the tenant's ABC accounts + ship-tos + branches
    // and persist into abc_ship_to_accounts / abc_account_branches / abc_branches
    // so the catalog hook (useAbcCatalog) has real data after Connect.
    if (action === "sync_accounts") {
      ...
      // Normalize ABC's account shape. Different ABC envs return slightly different
      // keys; we pull every plausible ship-to identifier off each row.
      ...
      // 2) For each ship-to, GET /shiptos/{shipToNumber} to get branches + address.
      ...
      // 4) Upsert abc_account_branches (ship_to_id + branch_number unique).
      ...
      // 5) Upsert abc_branches (tenant + branch_number unique).
    ...
    if (action === "get_order_status") {
      ...
      if (r.ok) {
        try {
          ...
          console.warn("[abc-api-proxy] get_order_status persist failed", e);
        }
      }
    ...
    if (action === "submit_test_order" || action === "validate_payload_only") {
      ...
      try {
        ...
        console.warn("[abc-api-proxy] submit_test_order persist failed", persistErr);
      }
    ...
    if (action === "place_order" || action === "submit_order") {
      ...
      if (body.order) {
        ...
        // ── UOM gate ────────────────────────────────────────────────
        // ABC branches reject orders with invalid UOMs. The UOM MUST come
        // from the Product API response for the selected item — never a
        // free-text default. Reject early so the dialog can prompt the user.
        const missingUom = items.find((i) => !String(i.unit || "").trim());
        if (missingUom) {
          return json({
            success: false,
            error: "missing_item_uom",
            interpretation: `Item "${missingUom.item_name || missingUom.abc_item_code}" is missing a UOM. Pull the UOM from the Product API for the selected item and resubmit.`,
          }, 400);
        }
        ...
        // ── Price echo ──────────────────────────────────────────────
        // ABC branches expect the order to include the price we showed the
        // customer (which should match what they see in their system). If a
        // line came in without `unit_cost`, resolve it server-side via
        // /pricing/v2/prices so we never submit a $0 placeholder.
        const ts = Date.now();
        const priceMap = new Map<string, number>();
        const needsPrice = items.filter((i) => !(Number(i.unit_cost) > 0));
        if (needsPrice.length && shipToNumber && branchNumber) {
          ...
          } catch (_e) { /* non-fatal — order will submit with the costs we have */ }
        }
        ...
        // ── Jobsite contact (DC) ───────────────────────────────────
        // Branches need a jobsite contact (functionCode "DC") so the driver
        // knows who to call on delivery. Sourced from `body.jobsite_contact`
        // which the dialog populates from the project's primary contact.
