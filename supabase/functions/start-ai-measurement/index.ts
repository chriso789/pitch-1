...
// ----------------------------------------------------------------------------
// Every persisted write of geometry_report_json MUST carry the new DSM
// diagnostic fields (source/failure_reason/derived/warning/confidence/tokens),
// even when bounds are absent. Without this, the live row keeps the legacy
// "field-absent" shape and the UI renders blanks where the runtime has answers.
// This helper is fully idempotent: it never overwrites real values.
// ─────────────────────────────────────────────────────────────────────────────
const DSM_DIAGNOSTIC_PROPAGATION_VERSION = "dsm-diagnostic-propagation-v1";

const DSM_DIAGNOSTIC_FIELDS = [
...
function summarizeDsmDiagnosticReason(reg: Record<string, unknown>): string {
  ...
  if (reg.dsm_bounds_derived === true) return "bounds_derived_lower_confidence";
...
function ensureDsmDiagnosticsOnRegistration(
  ...
  // (1) Run the hoist when registration looks incomplete. Idempotent; the
  // hoist itself short-circuits when DSM fields are already present.
  try {
  ...
  // "field-absent". Null means "the runtime tried and has no value" — not
  ...
  // registration. Do not overwrite if registration already carries one.
  if (
  ...
  // (4) Human-readable summary for the UI and for grepping the row.
...
function registrationInputFromBlock(
  original: RegistrationGateInput,
...
function applyLiveRuntimeHoistToRegistration(
  ...
  try {
    ...
    reg.dsm_hoist_callsite = "start-ai-measurement";
    ...
    if (!candidateAlreadyHoisted) {
      ...
      if (staleOnly) {
        ...
        reg.candidate_source_status = "current_run";
      }
    ...
    if (reg.selected_candidate_polygon_px_present) {
      const stageReport = classifyRegistrationStage({
        ...
        dsm: {
          ...
          dsm_decode_success: reg.dsm_stage_attempted === true,
      ...
      reg.stage_failure_stage = stageReport.failure_stage;
      reg.coordinate_space_audit = stageReport.coordinate_space_audit;
    }
    return reg;
  } catch (e) {
    console.warn(
...
function prepareRoofMeasurementPayload(
  ...
  if (regInput) {
    try {
      ...
      (geometry as any).registration_gate = registrationBlock;
  ...
  try {
    ...
    let dsmCoordinateMatchDebug: any = null;
    {
      ...
      if (!dsmCoordinateMatch) {
        // ── Aerial-Primary downgrade pre-check ────────────────────────────
        // Footprint is valid; only DSM coordinate registration failed. If we
        // have a registered aerial perimeter + raster transform, persist as
        // `perimeter_only` (not `ai_failed_runtime`) so the report can render
        // the aerial graph while remaining customer-report-blocked.
        {
          ...
          if (_ap.aerial_primary_ready) {
            const dsmReason = "dsm_coordinate_registration_failed";
            const debugPayload = {
              topology_source: REQUIRED_TOPOLOGY_SOURCE,
              footprint_source: footprintSource,
              footprint_valid: true,
            ...
            await setMeasurementJobStatus(
              input.measurement_job_id,
              "needs_review",
              "Aerial perimeter accepted; DSM validation unavailable",
              failedId,
            );
            await setAiJobStatus(
              input.ai_measurement_job_id,
    ...
    const geometryReportJson = {
      ...
      perimeter_phase0: autonomousDebug?.perimeter_phase0 ?? null,
      perimeter_ready: autonomousDebug?.perimeter_ready ?? false,
      perimeter_source: autonomousDebug?.perimeter_source ?? null,
      perimeter_gate_passed: autonomousDebug?.perimeter_gate_passed ?? null,
      perimeter_eave_ft: autonomousDebug?.perimeter_eave_ft ?? null,
      perimeter_rake_ft: autonomousDebug?.perimeter_rake_ft ?? null,
...
async function insertFailedPreliminaryMeasurement(
  ...
  const geometryReportJson = {
    ...
    perimeter_source: debug?.perimeter_source ?? null,
    perimeter_gate_passed: debug?.perimeter_gate_passed ?? null,
    perimeter_eave_ft: debug?.perimeter_eave_ft ?? null,
    perimeter_rake_ft: debug?.perimeter_rake_ft ?? null,
    perimeter_total_ft: debug?.perimeter_total_ft ?? null,
    ...
    footprint_area_px: debug?.footprint_area_px ?? 0,
    footprint_area_sqft: debug?.footprint_area_sqft ?? 0,
    footprint_coordinate_space: debug?.footprint_coordinate_space || "pixel",
    dsm_edge_coordinate_space: debug?.dsm_edge_coordinate_space || "pixel",
    coordinate_space_match: debug?.coordinate_space_match ?? true,
    raw_edges: debug?.raw_edges ?? debug?.dsm_edges_detected ?? 0,
    accepted_edges: debug?.accepted_edges ?? debug?.dsm_edges_accepted ?? 0,
    clustered_edges: debug?.clustered_edges ?? 0,
    graph_nodes: debug?.graph_nodes ?? 0,
    ...
    debug_geometry: {
      ...
      interior_lines_used: Number(debug?.interior_lines_used || 0),
      graph_nodes: Number(debug?.graph_nodes || 0),
      graph_segments: Number(debug?.graph_segments || 0),
