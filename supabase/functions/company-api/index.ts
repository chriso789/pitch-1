// company-api — routed Edge Function.
...
const app = createRouter("company-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "company-api", ok: true }));

// Apply auth + tenant guard to everything else unless explicitly public.
app.use("/*", requireAuth);
app.use("/*", requireTenant);

app.post("/initialize", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.post("/user/create", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.post("/tenant-owner/provision", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
