// security-api — routed Edge Function.
...
const app = createRouter("security-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "security-api", ok: true }));

// Apply auth + tenant guard to everything else unless explicitly public.
app.use("/*", requireAuth);
app.use("/*", requireTenant);

app.post("/audit", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.post("/scan", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
