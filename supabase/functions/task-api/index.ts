...
const app = createRouter("task-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "task-api", ok: true }));

// Apply auth + tenant guard to everything else unless explicitly public.
app.use("/*", requireAuth);
app.use("/*", requireTenant);

app.post("/assign", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
