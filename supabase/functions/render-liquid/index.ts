...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Pass Auth header through so RLS applies to all reads/writes
    ...
    if (body.lead_id) {
      const { data, error } = await supabase.from('pipeline_entries').select('*').eq('id', body.lead_id).single()
    ...
    if (body.estimate_id) {
      const { data, error } = await supabase.from('enhanced_estimates').select('*').eq('id', body.estimate_id).single()
    ...
    const rendered_text = await engine.parseAndRender(doc.body, ctx)

    // Extract variables referenced by the template (best-effort)
    let referenced: string[] = []
    try {
      const parsed = engine.parse(doc.body)
      // @ts-ignore - LiquidJS type definitions may vary
...
function getByPath(obj: any, path: string): any {
  // Supports dot access only, e.g., contact.first_name
