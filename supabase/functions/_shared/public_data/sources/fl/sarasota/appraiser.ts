...
export const flSarasotaAppraiser: AppraiserAdapter = {
  ...
  async lookupByAddress(input: { loc: NormalizedLocation; county: CountyContext; timeoutMs: number }) {
    ...
    try {
      const searchUrl = `${baseUrl}/search?address=${encodeURIComponent(address)}`;
      console.log(`[appraiser:${key}] Scraping: ${searchUrl}`);
      ...
      if (jsonData.last_sale_date) res.last_sale_date = jsonData.last_sale_date;
