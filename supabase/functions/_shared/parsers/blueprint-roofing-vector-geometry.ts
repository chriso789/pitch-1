...
export const ROOFING_VECTOR_GEOMETRY_VERSION = "roofing-vector-geometry-v1";
...
function viewportAnchor(v: DrawingViewport): Pt {
  return { x: v.bbox.x + v.bbox.width / 2, y: v.bbox.y + v.bbox.height / 2 };
}
...
function isRoof(v: DrawingViewport, page: PdfLayoutPage): boolean {
  return ROOF_RE.test(`${v.title ?? ""} ${v.metadata.title_item_text ?? ""}`) || (viewportsOnPage(page).length===1 && /\bROOF\s+PLAN\b/i.test(page.text));
}
function viewportsOnPage(_page: PdfLayoutPage): DrawingViewport[] { return []; }
...
function dimensionAnchors(
  ...
  for (const dim of dimensions) {
    ...
    if (!chosen) continue;
    out.push({ known_feet: dim.normalized_feet, pdf_distance_points: chosen.length_points, source_text: dim.label_text, confidence: Math.min(0.75, dim.confidence*0.8) });
  }
...
function bbox(points:Pt[]){const xs=points.map(p=>p.x),ys=points.map(p=>p.y);return {x:Math.min(...xs),y:Math.min(...ys),width:Math.max(...xs)-Math.min(...xs),height:Math.max(...ys)-Math.min(...ys)};}
...
export function buildRoofingVectorGeometry(input:{pages:PdfLayoutPage[];viewports_by_page:Map<number,DrawingViewport[]>|Record<number,DrawingViewport[]>;dimensions?:DimensionCandidate[]}):RoofingVectorGeometryResult{
  ...
  for(const page of input.pages){
    ...
    for(const viewport of vs){
      ...
      const anchors=dimensionAnchors(viewport,input.dimensions??[],assigned);
      ...
      if(calibration.review_required) review_flags.push({flag_code:"ROOF_VIEWPORT_SCALE_REVIEW",severity:"warning",blocking:false,message:calibration.message,metadata:{page_number:page.page_number,viewport_key:viewport.viewport_key,status:calibration.status}});
