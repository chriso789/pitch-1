...
export type SafeRoofingTakeoffInput = RoofingTakeoffInput & {
  symbol_candidates?: RoofingGraphicSymbolInput[];
};

function centerText(item: { x:number;y:number;width:number;height:number }) {
...
function hasNearbyExplicitLabel(input: SafeRoofingTakeoffInput, symbol: RoofingGraphicSymbolInput): boolean {
  const page = input.pages.find((p) => p.page_number === symbol.page_number);
  if (!page) return false;
  const patterns: Record<RoofSymbolCandidate["kind"], RegExp> = {
    roof_drain: /\b(?:RD[-\s]?\d+|ROOF\s+DRAIN)\b/i,
    scupper: /\bSCUPPER\b/i,
    curb: /\b(?:RTU|AHU|EF|CU|ROOF\s+CURB)\b/i,
    penetration: /\b(?:VTR|ROOF\s+VENT|PIPE\s+PENETRATION|PIPE\s+BOOT)\b/i,
  };
...
function appendGraphicSymbolCounts(result: RoofingTakeoffResult, input: SafeRoofingTakeoffInput) {
  ...
  for (const [kind, rows] of Object.entries(Object.groupBy(candidates, (s) => s.kind))) {
    const list = rows ?? [];
    if (!list.length) continue;
    const m = mapping[kind as keyof typeof mapping];
    ...
    for (const [index, symbol] of list.entries()) {
      ...
      result.plan_paths.push({
        ...
        source_coordinates: { bbox: symbol.bbox, center: symbol.center },
    ...
    if (existing) {
      ...
      existing.metadata = {
        ...(existing.metadata ?? {}),
        graphic_symbol_count: list.length,
        graphic_symbol_plan_path_keys: pathKeys,
      ...
      result.measurements.push({
        ...
        measurement_group: m.group,
        quantity: list.length,
        unit: "count",
        precision: 0,
        confidence: Math.min(...list.map((s) => s.confidence)),
        source_value_raw: `${list.length} graphic ${m.label} candidate(s)`,
        normalized_value: { label_type: m.label, graphic_symbol_count: list.length },
        page_number: list.length === 1 ? list[0].page_number : null,
        metadata: {
          engine_version: "roofing-blueprint-v1+graphic-symbols",
          review_required: true,
          count_source: "roof_graphic_symbol",
          graphic_symbol_plan_path_keys: pathKeys,
        },
      });
    }
    result.review_flags.push({
      flag_code: "ROOF_GRAPHIC_SYMBOL_COUNT_REVIEW",
      severity: "warning",
      blocking: false,
      message: `${list.length} ${m.label} count candidate(s) came from graphic-symbol recognition and require review.`,
      metadata: { kind, count: list.length, plan_path_keys: pathKeys },
    });
  }
