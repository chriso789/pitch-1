...
Deno.test("closed roof vectors convert to expected area using viewport scale",()=>{
  ...
  assertEquals(vector.evidence.find(e=>e.geometry_class==="ridge")?.length_ft,10);
  ...
  assertEquals(takeoff.measurements.find(m=>m.measurement_key==="ridges_lf")?.quantity,10);
