...
// Enterprise-grade comprehensive onboarding email template
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[send-company-onboarding] Sending enterprise onboarding email to ${email} for ${company_name}`);
    ...
    if (customTemplate?.html_body) {
      ...
      console.log('[send-company-onboarding] Using enterprise 5-step email template');
      emailHtml = generateEnterpriseEmailHtml(first_name, company_name, onboardingUrl);
        ...
        try {
          const emailResult = await resend.emails.send({
            ...
            tags: [
              ...
              { name: "campaign", value: "enterprise_onboarding_v3" }
            ],
          });
    ...
    console.error("[send-company-onboarding] Error:", error);
    return new Response(
