// health-api — routed Edge Function.
...
const app = createRouter("health-api");

// Health probe (always public)
app.get("/__health", (c) => jsonOk(c, { fn: "health-api", ok: true }));
...
app.get("/", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.get("/db", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
app.get("/version", async (c) => jsonErr(c, "not_migrated", "Route scaffolded; logic not yet migrated.", 501));
