...
BEGIN
  ...
  RETURN QUERY
  -- CONTACTS (tenant-wide, no location filter)
  SELECT 
  ...
  -- LEADS (non-project pipeline entries, company-wide)
  SELECT 
    'lead'::text AS entity_type,
    pe.id AS entity_id,
    COALESCE(pe.lead_name, c.first_name || ' ' || c.last_name, c.first_name, c.last_name, pe.clj_formatted_number, 'Unnamed Lead')::text AS entity_name,
  ...
  -- JOBS (pipeline entries with status = 'project', company-wide)
  SELECT 
    'job'::text AS entity_type,
    pe.id AS entity_id,
    COALESCE(pe.lead_name, c.first_name || ' ' || c.last_name, c.first_name, c.last_name, pe.clj_formatted_number, 'Unnamed Job')::text AS entity_name,
    COALESCE(c.address_street, c.email, '')::text AS entity_subtext,
    COALESCE(pe.clj_formatted_number, '')::text AS clj_number,
    COALESCE(pe.status::text, 'project')::text AS entity_status,
    CASE 
      WHEN lower(COALESCE(pe.clj_formatted_number, '')) LIKE lower(p_search_term) || '%' THEN 100
      WHEN lower(COALESCE(pe.lead_name, '')) LIKE lower(p_search_term) || '%' THEN 97
      WHEN lower(COALESCE(c.first_name || ' ' || c.last_name, '')) LIKE lower(p_search_term) || '%' THEN 95
