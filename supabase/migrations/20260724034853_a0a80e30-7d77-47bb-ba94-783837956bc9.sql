...
BEGIN
  ...
  SELECT * FROM (
    ...
    SELECT 'lead'::text, pe.id,
      ...
      -- Rewrite the stale prefix with the entry's CURRENT location code so
      -- badges reflect where the record lives now (e.g. WC-…), not where the
      -- contact was originally created.
      COALESCE(
        CASE
