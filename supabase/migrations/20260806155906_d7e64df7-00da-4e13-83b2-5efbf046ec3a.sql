...
DECLARE
  ...
  v_co_material numeric := 0;
  v_co_labor numeric := 0;
  v_line_items_text text := '';
BEGIN
  ...
  IF v_selected_estimate_id IS NOT NULL THEN
    SELECT 
      ...
      COALESCE(line_items::text, '')
    INTO v_materials, v_labor, v_overhead, v_profit, v_base_sale_price, v_margin_pct, v_sales_tax_amount, v_line_items_text
  ...
  -- Approved / invoiced / completed change orders for ANY project under this pipeline entry.
  -- Turnkey treatment: cost_impact (budget) adds to the selling price, and the change
  -- order's own material/labor spend adds to the cost side so margin stays honest.
  -- IMPORTANT: skip change orders that were already merged into the active estimate
  -- (the invoice-CO flow pushes their line items + totals into enhanced_estimates),
  -- otherwise the amount would be counted twice.
  SELECT
    COALESCE(SUM(COALESCE(co.cost_impact, 0)), 0),
    COALESCE(SUM(COALESCE(co.material_total, 0)), 0),
    COALESCE(SUM(
      CASE
        WHEN COALESCE(co.material_total, 0) + COALESCE(co.labor_total, 0) > 0
          THEN COALESCE(co.labor_total, 0)
        ELSE COALESCE(co.cost_impact, 0)
      END
    ), 0)
  INTO v_co_total, v_co_material, v_co_labor
  ...
  WHERE p.pipeline_entry_id = p_pipeline_entry_id
    ...
    AND POSITION(co.id::text IN v_line_items_text) = 0;

  v_materials := v_materials + v_co_material;
  v_labor := v_labor + v_co_labor;
  ...
  v_result := json_build_object(
    ...
    'change_orders_material', v_co_material,
    'change_orders_labor', v_co_labor,
