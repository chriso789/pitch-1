-- Drop and recreate api_estimate_hyperlink_bar to read costs from selected enhanced_estimate
...
BEGIN
  ...
  -- Check materials order status
  SELECT CASE WHEN EXISTS (
    SELECT 1 FROM public.material_orders 
    WHERE pipeline_entry_id = p_estimate_id AND status IN ('ordered', 'delivered')
  ) THEN 'complete' ELSE 'pending' END
  INTO v_materials_status;

  -- Check labor assignment status
  SELECT CASE WHEN EXISTS (
