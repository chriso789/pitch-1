...
DROP FUNCTION IF EXISTS public.search_contacts_and_jobs(uuid, text, uuid);
DROP FUNCTION IF EXISTS public.search_contacts_and_jobs(uuid, text);

-- Create the corrected function using PROJECTS instead of JOBS
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(c.address, c.email, c.phone, '')::text AS entity_subtext,
    ''::text AS clj_number,
    COALESCE(c.status, 'active')::text AS entity_status,
    CASE 
  ...
  WHERE c.tenant_id = p_tenant_id
    ...
    AND (
      ...
      OR lower(COALESCE(c.address, '')) LIKE v_search_pattern
    )
  ...
  SELECT 
    ...
    COALESCE(c.address, c.email, '')::text AS entity_subtext,
  ...
  -- PROJECTS (replaces JOBS - projects table has clj_formatted_number)
  SELECT 
    'job'::text AS entity_type,
    p.id AS entity_id,
    COALESCE(p.name, p.clj_formatted_number, 'Unnamed Project')::text AS entity_name,
    COALESCE(c.first_name || ' ' || c.last_name, c.address, '')::text AS entity_subtext,
