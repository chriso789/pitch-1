...
-- 1) Data hygiene
UPDATE public.contacts SET is_deleted = true WHERE deleted_at IS NOT NULL AND is_deleted = false;

ALTER TABLE public.pipeline_entries DISABLE TRIGGER USER;
UPDATE public.pipeline_entries SET is_deleted = true WHERE deleted_at IS NOT NULL AND is_deleted = false;
ALTER TABLE public.pipeline_entries ENABLE TRIGGER USER;

-- 2) Harden search RPC
CREATE OR REPLACE FUNCTION public.search_contacts_and_jobs(
  p_tenant_id uuid, p_search_term text, p_location_id uuid DEFAULT NULL
) RETURNS TABLE(
  entity_type text, entity_id uuid, entity_name text, entity_subtext text,
  clj_number text, entity_status text, match_score integer, project_type text
