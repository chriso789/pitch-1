...
BEGIN
  ...
  INSERT INTO public.abc_tokens (
    ...
    CASE WHEN p_refresh_token IS NULL THEN NULL ELSE extensions.pgp_sym_encrypt(p_refresh_token, p_enc_key) END,
...
BEGIN
  ...
  RETURN QUERY
  SELECT
    extensions.pgp_sym_decrypt(t.access_token_enc, p_enc_key)::text,
    CASE WHEN t.refresh_token_enc IS NULL THEN NULL
         ELSE extensions.pgp_sym_decrypt(t.refresh_token_enc, p_enc_key)::text END,
    t.token_type,
    t.scope,
