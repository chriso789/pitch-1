-- Fix api_estimate_hyperlink_bar to use project_id for purchase_orders and labor_cost_tracking
-- These tables use project_id, not pipeline_entry_id
...
BEGIN
  ...
  -- Get project_id linked to this pipeline entry (for purchase_orders/labor lookups)
  ...
  -- Check materials order status using purchase_orders with project_id
  ...
  -- Check labor assignment status using labor_cost_tracking with project_id
  IF v_project_id IS NOT NULL THEN
    SELECT CASE WHEN EXISTS (
