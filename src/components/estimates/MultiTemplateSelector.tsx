...
export const MultiTemplateSelector: React.FC<MultiTemplateSelectorProps> = ({
  ...
  const handleTradeTemplateSelect = async (tradeSectionId: string, tradeType: string, templateId: string, isPrimaryRoofing: boolean = true) => {
    ...
    try {
      ...
      // Auto-repair: if template has 0 items, copy from a same-named reference template
      if (!data || data.length === 0) {
        console.log(`[TEMPLATE_REPAIR] Trade ${tradeType} template has 0 items, looking for reference...`);
        const { data: thisTemplate } = await supabaseClient
          .from('estimate_calculation_templates')
          .select('name')
          .eq('id', templateId)
        ...
        if (thisTemplate?.name) {
          ...
          for (const ref of (referenceTemplates || [])) {
            ...
            if (refItems && refItems.length > 0) {
              console.log(`[TEMPLATE_REPAIR] Copying ${refItems.length} items from reference ${ref.id}`);
              const itemsToInsert = refItems.map((item: any) => ({
                ...item,
                calc_template_id: templateId,
              }));
  ...
  const handleSaveLineItemChanges = async () => {
    ...
    try {
      ...
      const described = await describeEdgeFunctionError('update-estimate-line-items', error);
      toast({
