...
interface TextBlastListBuilderProps {
  open: boolean;
...
export const TextBlastListBuilder: React.FC<TextBlastListBuilderProps> = ({
  open,
  onOpenChange,
  onListCreated,
}) => {
  ...
  const { data: contacts, isLoading } = useQuery({
    queryKey: ['text-blast-list-builder-contacts', tenantId, statusFilter, sourceFilter, stagnantDays],
    queryFn: async () => {
      if (!tenantId) return [];
      let query = supabase
        .from('contacts')
      ...
      if (stagnantDays) {
        query = query.lt('created_at', subDays(new Date(), stagnantDays).toISOString());
      }
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Build Text Blast List
          </DialogTitle>
          <DialogDescription>
            Filter and select contacts to create a text blast list. Contacts with active projects are excluded.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 flex-1 min-h-0">
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g. Spring Follow-Up Leads" value={listName} onChange={e => setListName(e.target.value)} />
          </div>
