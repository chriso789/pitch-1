...
export function ABCConnectionSettings() {
  ...
  // ────────────────────────────────────────────────────────────────────
  // G. Order Tracking (SRS-style)
  // ────────────────────────────────────────────────────────────────────
  const orderLines: any[] = trackingFirst?.lines || orderResult?.orderRequest?.[0]?.lines || [];
  const trackingStatus = trackingFirst?.status || trackingFirst?.orderStatus || (orderResult?.success ? 'submitted' : null);
  const hasTracker = !!(orderResult?.orderNumber || orderResult?.confirmationNumber);

  const OrderTrackingCard = (orderResult || trackResult) && (
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="font-mono">ABC Supply</Badge>
            <CardTitle className="text-lg">{orderResult?.purchaseOrder || trackingFirst?.purchaseOrder || 'ABC Order'}</CardTitle>
          </div>
          <div className="flex items-center gap-2">
            {trackingStatus && (
              <Badge variant={orderResult?.success || trackResult?.success ? 'default' : 'destructive'}>
                {trackingStatus}
              </Badge>
            )}
            {(orderResult?.timestamp || trackResult?.timestamp) && (
              <Badge variant="outline" className="text-[10px]">
                {new Date(orderResult?.timestamp || trackResult?.timestamp).toLocaleString()}
              </Badge>
            )}
            <Button
              size="sm"
      ...
      <CardContent className="space-y-3">
        {!hasTracker && orderResult && (
          <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-amber-800 dark:text-amber-300">
            Sandbox order submitted. ABC did not return an order/confirmation number in this response. Use the request ID below for follow-up.
          </div>
        )}
        {orderResult?.success === false && (
          <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 text-xs space-y-1">
            ...
            <div className="text-muted-foreground break-words">
              {orderResult.interpretation ||
                orderResult.error ||
                `ABC responded HTTP ${orderResult.orderResponse?.status ?? '—'}`}
            </div>
          </div>
        )}

        <div className="grid gap-2 md:grid-cols-2 text-xs">
          <EndpointRow label="ABC Order #" value={orderResult?.orderNumber || trackingFirst?.orderNumber || ''} />
          <EndpointRow label="Confirmation #" value={orderResult?.confirmationNumber || trackingFirst?.confirmationNumber || ''} />
          <EndpointRow label="Request ID" value={orderResult?.requestId || trackingFirst?.requestId || ''} />
          <EndpointRow label="Purchase Order" value={orderResult?.purchaseOrder || trackingFirst?.purchaseOrder || ''} />
          <EndpointRow label="Branch" value={branchNumber} />
          <EndpointRow label="Ship-To" value={shipToNumber} />
        </div>
        ...
        {orderLines.length > 0 && (
          <div className="rounded border overflow-hidden">
            <Table>
              ...
              <TableBody>
                {orderLines.map((l: any, i: number) => (
                  <TableRow key={i}>
                    <TableCell className="font-mono text-xs">{l.itemNumber || '—'}</TableCell>
                    <TableCell className="text-xs">{l.itemDescription || l.description || '—'}</TableCell>
                    <TableCell className="text-right text-xs">{l.orderedQty?.value ?? l.quantity ?? '—'}</TableCell>
                    <TableCell className="text-xs">{l.orderedQty?.uom ?? l.uom ?? 'EA'}</TableCell>
        ...
        <Accordion type="single" collapsible>
          <AccordionItem value="inspect" className="border-none">
            <AccordionTrigger className="text-xs py-2 hover:no-underline">Inspect raw request/response</AccordionTrigger>
            <AccordionContent>
              <div className="space-y-2">
                {orderResult?.orderRequest && (
                  <div>
                    <div className="text-[11px] font-medium text-muted-foreground mb-1">Request payload (array)</div>
                    <pre className="font-mono text-[10px] bg-muted/40 p-2 rounded overflow-x-auto max-h-64">
                      {JSON.stringify(orderResult.orderRequest, null, 2)}
                    </pre>
                  </div>
                )}
                {(orderResult?.orderResponse || trackResult?.body) && (
                  <div>
                    <div className="text-[11px] font-medium text-muted-foreground mb-1">ABC response</div>
                    <pre className="font-mono text-[10px] bg-muted/40 p-2 rounded overflow-x-auto max-h-64">
                      {JSON.stringify(orderResult?.orderResponse?.body ?? trackResult?.body, null, 2)}
                    </pre>
                  </div>
                )}
              </div>
