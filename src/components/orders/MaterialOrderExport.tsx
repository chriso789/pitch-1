...
export function MaterialOrderExport({ order, items, projectAddress, companyInfo }: MaterialOrderExportProps) {
  ...
  const generatePDF = () => {
    try {
      ...
      doc.setFontSize(12);
      doc.text(`PO# ${order.po_number}`, margin, 30);
      ...
      // TO Section (Vendor Info)
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.text('TO (Vendor):', margin, yPos);
      yPos += 8;
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      ...
      const jobSiteAddress = projectAddress || order.delivery_address || order.project_address;
      if (jobSiteAddress) {
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.text('Job Site / Delivery Address', margin, yPos);
        yPos += 8;
        ...
        doc.text(jobSiteAddress, margin + 5, yPos + 5);
