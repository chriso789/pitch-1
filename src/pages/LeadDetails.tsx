...
// (ProjectFinancialSections merged into ProfitCenterPanel)
        ...
        return (
          <div className="space-y-4">
            {projectData?.project?.id && (
              <ProjectAccountingPanel projectId={projectData.project.id} />
            )}
            <ProfitCenterPanel pipelineEntryId={id!} projectId={projectData?.project?.id} contactId={lead?.contact?.id} />
          </div>
        );
      case 'total':
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-3 md:space-y-6 p-2 md:p-6 pb-32 md:pb-16">
      ...
      <div className="space-y-1.5 text-xs md:text-sm">
        ...
        {/* Property Address panel removed — address is validated during lead/contact creation */}
        ...
        {lead.contact && (
          <div className="flex items-center gap-2 px-1">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="h-6 text-xs px-2 gap-1 font-medium"
      ...
      {/* Project financial data is now in the Profit tab */}
